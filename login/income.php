...
while (!$unique_ref_found) {  
    ...
    $query = mysqli_query($con, "SELECT `inc_id` FROM ` income_tbl` 
              WHERE `inc_id`='".$unique_ref."'");  
...
<input name="inc_id" type="hidden" value="<?php echo $playernumber; ?>">

<div class="form-group">							  <label>Income Date</label>

                    <input type="date" class="form-control" name="inc_date" id="lname" placeholder="Select Date for Income" required />
				</div>

<div class="form-group">							  <label>Income Description</label>

					<input type="text" onkeyup="this.value = this.value.toUpperCase();" pattern="^[A-Za-z0-9 -]+$" class="form-control" name="inc_des" id="lname" placeholder="Describe the Income" required />
				</div>


<div class="form-group">							  <label>Amount (e.g. 1234567890)</label>

					<input type="text" onkeyup="this.value = this.value.toUpperCase();" pattern="^[0-9 -]+$" class="form-control" name="amount" id="lname" placeholder="Enter The Amount" required />
				</div>
                ...
                <input name="income" type="checkbox" value="agree" required> I agree to Cooperative Terms and Condition
				</div>
