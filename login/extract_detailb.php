...
if ($_POST['extractsave'] <> ""){
		@$dateo = trim($_POST['dateo']);
		@$dateo = strip_tags($dateo);
		@$dateo = htmlspecialchars($dateo);
		@$_SESSION['dateo'] = @$dateo;
	header("Location: reprint_deduction.php");
}
else{
	header("Location: select_reportb.php");
		exit;}
...
<div style="background-image: url(../onlineapp/ccslogo/header.jpg); background-repeat: no-repeat center center; background-size: cover; height: 150px"></div>
	<div class="container"><br>
	  ...
	  <h4 align="center">EXTRACTED REPORT DETAILS</h4><br><table width="100%" border="0" class="table table-striped">
  <tr>
    <td colspan="3">EXTRACTED RECORD</td>
    </tr>
  <tr>
    <td width="12%"><strong>No.</strong></td>
    <td width="30%"><strong>Title</strong></td>
    <td width="21%"><strong>View</strong></td>
  </tr>
<?php $no='0';
$conn = mysqli_connect('localhost','root','','cooperative');
$cust = mysqli_query($conn,"SELECT * FROM $table WHERE print_date<=$from_date and print_date>=$to_date"); 
while ($custrow=mysqli_fetch_array($cust)){?>  
<?php
if ($level=='Savings'){$print_id=$custrow['savings_id']; $page='print_deduction2.php';} 
if ($level=='Loans'){$print_id=$custrow['loan_id']; $page='print_mandate2.php';}
$no = $no + 1;
?>
...
<tr>
    ...
    <td><a href="<?php echo $page; ?>?page=null&print_id=<?php echo $print_id; ?>" title="<?php echo $print_id; ?>">-|-</a></td>
  </tr> <?php }?>
</table><br><br></div>
