jQuery(document).ready(function(){                
        ...
        jQuery("select[name='state']").change(function(){    
            ...
            var optionValue = jQuery("select[name='state']").val();        
switch (state) {
  case "Abia":
 ...
 case "Adamawa":
  ...
  var data =   ['Select item...', 'Anka', 'Bakura', 'Birnin Magaji Kiyaw', 'Bukkuyum', 'Bungudu', 'Gummi', 'Gusau', 'Kaura Namoda', 'Maradun', 'Maru', 'Shinkafi', 'Talata Mafara', 'Chafe', 'Zurmi'];
            ...
            jQuery("#lga")
                .html(ajaxLoader)
                .load('form.php', "state="+optionValue+"&status=1", function(response){
                    if(response) {
                        jQuery("#lga").css('display', '');
                    } else {
                        jQuery("#lga").css('display', 'none');
                    }
            });            
        });
    });